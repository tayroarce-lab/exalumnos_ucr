...
export default async function DirectorioEstudiantesPage(
  ...
  const { filtros, pagina, busqueda } = parseSearchParams(searchParams);
  const { estudiantes, total } = await getEstudiantes(filtros, { busqueda, page: pagina, limit: 12 });

  return (
    <div className="min-h-screen bg-slate-50/50 p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl border border-slate-100 w-full relative overflow-hidden text-slate-900">
